...
<div class=" dashboard">
  ...
  <div class="admin-controller  ">
                        ...
                        <div class="row"> 
                            ...
                            <form class="form-horizontal" method="post" action="{{ route('PostFeed') }}" >
    @csrf
    ...
    <div class="row   ">
        ...
        <span class="text-danger">{{ $errors->first('name') }}</span>
			...
			<select name="language" required class="form-control {{ $errors->has('language') ? 'has-error' : '' }}" id="language">
              ...
              <option value="turkish" @if(old('language')=='turkish') selected @endif>Turkish</option>
              <option value="german" @if(old('language')=='german') selected @endif>German</option>
          ...
          <span class="text-danger">{{ $errors->first('language') }}</span>
            ...
            <select name="category" required class="form-control {{ $errors->has('category') ? 'has-error' : '' }}" id="language">
          ...
          <span class="text-danger">{{ $errors->first('language') }}</span>
            ...
            <input type="text" name="feed_url" {{ old('feed_url') }} required class="form-control {{ $errors->has('feed_url') ? 'has-error' : '' }}" id="feed_url" placeholder="Enter feed url">
          <span class="text-danger">{{ $errors->first('feed_url') }} @if(session('error')) {{session('error')}} @endif</span>
            
		 
			</div>

            <div class="col-md-12  mt-3 mb-2">
            <button type="submit" class="button  btn-success primary-button mt-2">Submit</button>
          <a href="{{route('Feed')}}"><button type="button" class="button  btn-default primary-button mt-2">Cancel</button></a>
    ...
    {{-- <div class="form-group ">
      ...
      <div class="col-sm-10">
        <input type="text" name="source" value="{{ old('source') }}" required class="form-control {{ $errors->has('source') ? 'has-error' : '' }}" id="source" placeholder="Enter source">
    ...
    {{-- <div class="form-group ">
      ...
      <div class="col-sm-10">
        <input type="text" name="language" value="{{ old('language') }}" required class="form-control {{ $errors->has('language') ? 'has-error' : '' }}" id="language" placeholder="Enter language">
        <span class="text-danger">{{ $errors->first('language') }}</span>
      </div>
    </div> --}}
      ...
      <table id="feedslist-table" class="feedslist-table dataTable no-footer "  style="width:100%;  ">
        <thead>
            {{-- <tr>
              ...
              <td></td>
              <td></td>
              <td><input type="text" id="search-date2" placeholder="search date2"></td>
        ...
        <tr>
            <td>{{$feed->name}}</td>
             <td>{{$feed->feed_url}}</td>
            <td>{{$feed->getCategory->category_name}}</td>
            <td>{{$feed->language}}</td>
            <td>
